...
import { Sprout, Plus, TrendingUp, Package, DollarSign, LogOut, Languages, User } from 'lucide-react';
...
const FarmerDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setCrops(cropsRes.data.filter(c => c.farmer_id === user?.user_id));
  ...
  const stats = [
    {
      icon: Package,
      label: t('Active Listings', 'सक्रिय सूची'),
      value: crops.length,
      color: 'text-[#15803d]',
      bg: 'bg-green-50'
    },
    ...
    },
    {
      ...
      value: `₹${crops.reduce((sum, c) => sum + (c.price * c.quantity), 0).toLocaleString()}`,
      color: 'text-[#22c55e]',
      bg: 'bg-green-50'
    }
  ];
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-7xl py-8">
        <div className="mb-8">
          ...
          <p className={`text-base text-slate-600 ${language === 'hi' ? 'hindi' : ''}`}>
            {t('Manage your crops and track market prices', 'अपनी फसलें प्रबंधित करें और बाजार कीमतें ट्रैक करें')}
          </p>
        </div>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className={`text-center py-12 text-slate-500 ${language === 'hi' ? 'hindi' : ''}`}>
              {t('No crops listed yet. Add your first crop!', 'अभी तक कोई फसल सूचीबद्ध नहीं। अपनी पहली फसल जोड़ें!')}
            </div>
          ) : (
